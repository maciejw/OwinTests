...
using Modified.Microsoft.Owin.StaticFiles.Infrastructure;
...
namespace Modified.Microsoft.Owin.StaticFiles
{
    ...
    internal class StaticFileContext
    {
        ...
        private bool _customETagNotFound;
        ...
        {
            IFileInfo fileInfo;
            bool found = _options.FileSystem.TryGetFileInfo(_subPath.Value, out fileInfo);
            if (found)
            {
                long length = GetLength(fileInfo);
                DateTime lastModified = GetLastModified(fileInfo);
                string lastModifiedString = lastModified.ToString(Constants.HttpDateFormat, CultureInfo.InvariantCulture);

                var etag = await _options.GetCustomETag(fileInfo);

                var customETagNotFound = string.IsNullOrEmpty(etag);
                if (customETagNotFound)
                {
                    etag = GetDefaultETag(length, lastModified);
                }

                _customETagNotFound = customETagNotFound;
                _fileInfo = fileInfo;
                _length = length;
                _lastModified = lastModified;
                _lastModifiedString = lastModifiedString;
                _etag = etag;
                _etagQuoted = '\"' + _etag + '\"';
            }
            return found;
        ...
        private static string GetDefaultETag(long length, DateTime lastModified)
        {
            long etagHash = lastModified.ToFileTimeUtc() ^ length;
            return Convert.ToString(etagHash, 16);
        }

        private static DateTime GetLastModified(IFileInfo fileInfo)
        {
            DateTime last = fileInfo.LastModified;
            // Truncate to the second.
            return new DateTime(last.Year, last.Month, last.Day, last.Hour, last.Minute, last.Second, last.Kind);
        }

        private static long GetLength(IFileInfo fileInfo)
        {
        ...
        public void ComprehendRequestHeaders()
        {
            ...
            if(_customETagNotFound) ComputeIfModifiedSince();
